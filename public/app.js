...
function localLogin(email, password) {
  ...
  const user = users.find((entry) => {
    ...
    const byPhone = normalizePhone(entry.phone || "") === identifierPhone;
...
function localRegister(payload) {
  ...
  if (!name || !email || !password || !phone) {
    throw new Error("Name, email, mobile number, and password are required");
  }
  if (phone.length !== 10) {
...
function localSendRegisterOtp(payload) {
  ...
  if (!name || !email || phone.length !== 10) {
    throw new Error("Name, valid email, and 10-digit mobile number are required");
  }
  const users = loadLocalUsers();
  const exists = users.find((entry) => entry.email.toLowerCase() === email);
  if (exists) {
