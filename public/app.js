...
function openWhatsappPayment(payload, checkoutItems, totals) {
  ...
  const text =
    ...
    `Total: ${encodeURIComponent(currency(totals.total))}`;
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (payload.method === "Online Pay (WhatsApp)") {
        openWhatsappPayment(payload, selectedItems, totals);
