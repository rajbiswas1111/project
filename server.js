...
async function handleApi(req, res, parsedUrl) {
  ...
  if (req.method === "POST" && pathname === "/api/auth/login") {
    ...
    const user = store.users.find(
      (entry) =>
        ...
        normalizePhone(entry.phone || "") === identifierPhone
    );
  ...
  if (req.method === "POST" && pathname === "/api/auth/register") {
    ...
    if (!name || !email || !password || !phone) {
      sendJson(res, 400, { error: "Name, email, mobile number, and password are required" });
      return;
    }
    if (phone.length !== 10) {
    ...
    const phoneExists = store.users.find(
      (entry) => normalizePhone(entry.phone || "") === phone
    );
    if (phoneExists) {
  ...
  if (req.method === "POST" && pathname === "/api/auth/register/send-otp") {
    ...
    if (!name || !email || phone.length !== 10) {
      sendJson(res, 400, { error: "Name, valid email, and 10-digit mobile number are required" });
      return;
    }
    const exists = store.users.find((entry) => entry.email.toLowerCase() === email);
    if (exists) {
    ...
    try {
      enforceOtpRateLimit(`register:${phone}`);
  ...
  if (req.method === "POST" && pathname === "/api/auth/reset-password/send-otp") {
    ...
    const identifier = String(body.identifier || body.email || "").trim().toLowerCase();
    if (!identifier) {
      sendJson(res, 400, { error: "Registered mobile number is required" });
      return;
    }
    const identifierPhone = normalizePhone(identifier);
    const user = store.users.find(
    ...
    try {
      ...
      if (targetPhone.length !== 10) {
        sendJson(res, 400, { error: "No valid mobile number on account" });
        return;
      }
      enforceOtpRateLimit(`reset:${targetPhone}`);
      const otp = createOtpRecord(otpStore.reset, user.email.toLowerCase());
      await sendOtpToPhone(targetPhone, otp);
      sendJson(res, 200, { success: true, message: "OTP sent" });
    } catch (error) {
      sendJson(res, 429, { error: error.message || "Failed to send OTP" });
    }
