    ...
    const seed = {
      users: [
        ...
        },
        {
          id: "user-1",
          name: "Priya Sharma",
          email: "customer@vastrasaree.com",
          password: "user123",
          role: "customer",
        },
      ],
      ...
      orders: [],
      settings: {
        brandName: "Vastra Saree House",
        supportEmail: "care@vastrasaree.com",
        supportPhone: "+91 98765 43210",
        address: "12 Heritage Lane, T Nagar, Chennai, India",
      },
    };
...
function sanitizeProduct(input, existing = {}) {
  return {
    id: input.id || existing.id || `SR${Date.now()}`,
...
async function handleApi(req, res, parsedUrl) {
  ...
  if (req.method === "PUT" && pathname.startsWith("/api/admin/products/")) {
    ...
    const body = await parseBody(req);
    store.products[index] = sanitizeProduct(body, store.products[index]);
    writeStore(store);
...
ensureStore();
server.listen(PORT, () => {
  console.log(`Vastra Saree House server running at http://localhost:${PORT}`);
});
